...
; 修改第286 312 313行 null 为 nullvar ，以便兼容H版。
; 增加212个 SCI_ 前缀常量。
...
class scintilla {
    ...
    __call(msg, ByRef wParam:=0, ByRef lParam:=0, params*)
    {
        ...
        else
        {
            ...
            if (lParam && !(lParam+0) && !isObject(lParam))
            {
            ...
            */
            ...
            switch, msg
            {
                case "GetText":
                    ...
                    , lParam := &lParam
                    , buf := true
                case "GetLine","GetCurLine":
                    VarSetCapacity(lParam, this.linelength(wParam)+1 * (a_isunicode ? 2 : 1))
                    , lParam := &lParam
                    , buf := true
                case "GetTextRange":
                    range := abs(wParam.1 - wParam.2)+1
                    , dSize :=  __sendEditor(this.hwnd, "GetLength")
                    , VarSetCapacity(lParam, range > dSize ? dSize : range)
            ...
            if (__isHexColor(lParam, msg))
                lParam := (lParam & 0xFF) <<16 | (lParam & 0xFF00) | (lParam >>16)
            if (__isHexColor(wParam, msg))
                wParam := (wParam & 0xFF) <<16 | (wParam & 0xFF00) | (wParam >>16)
            ...
            ; convert the text from ANSI
            if (buf)
            {
                lParam := StrGet((msg = "GetTextRange") ? blParam : &lParam, "UTF-8")
                , buf := false
            }
...
__sendEditor(hwnd, msg:=0, wParam:=0, lParam:=0){
    ...
    hwnd := !hwnd ? oldhwnd : hwnd
    , oldhwnd := hwnd
    , msg := !(msg+0) ? "SCI_" msg : msg
    ...
    return DllCall(df_%hwnd%            ; DIRECT FUNCTION
                  ...
                  ,"UInt" ,!(msg+0) ? %msg% : msg ; msg 是数字则直接用，否则将 msg 视为变量名取其值
...
; Global scintilla variables
{
    ...
    ; Main Scintilla Functions
    {
        ...
        ,SCI_GETSELECTIONEND:=2145,SCI_SETPRINTMAGNIFICATION:=2146,SCI_GETPRINTMAGNIFICATION:=2147
        ,SCI_SETPRINTCOLORMODE:=2148,SCI_SETPRINTCOLOURMODE:=2148,SCI_GETPRINTCOLORMODE:=2149,SCI_GETPRINTCOLOURMODE:=2149
        ,SCI_FINDTEXT:=2150,SCI_FORMATRANGE:=2151,SCI_GETFIRSTVISIBLELINE:=2152,SCI_GETLINE:=2153
        ...
        ,SCI_SETFOLDMARGINCOLOR:=2290,SCI_SETFOLDMARGINCOLOUR:=2290,SCI_SETFOLDMARGINHICOLOR:=2291,SCI_SETFOLDMARGINHICOLOUR:=2291
        ...
        ,SCI_GETEDGECOLUMN:=2360,SCI_SETEDGECOLUMN:=2361,SCI_GETEDGEMODE:=2362,SCI_SETEDGEMODE:=2363
        ,SCI_GETEDGECOLOR:=2364,SCI_GETEDGECOLOUR:=2364,SCI_SETEDGECOLOR:=2365,SCI_SETEDGECOLOUR:=2365
        ...
        ,SCI_GETLEXER:=4002,SCI_COLORISE:=4003,SCI_COLOURISE:=4003
        ,SCI_SETPROPERTY:=4004,SCI_SETKEYWORDS:=4005,SCI_SETLEXERLANGUAGE:=4006,SCI_LOADLEXERLIBRARY:=4007
    ...
    ; 从 3.5.1 的 scintilla.h 中扒出来的，上面缺少的带 SCI_ 前缀的常量
    {
        global SCI_STYLESETSIZEFRACTIONAL:=2061,SCI_STYLEGETSIZEFRACTIONAL:=2062,SCI_STYLESETWEIGHT:=2063
        ,SCI_STYLEGETWEIGHT:=2064,SCI_SETWHITESPACESIZE:=2086,SCI_GETWHITESPACESIZE:=2087
        ,SCI_CALLTIPSETPOSITION:=2213,SCI_CALLTIPSETPOSSTART:=2214,SCI_GETALLLINESVISIBLE:=2236
        ,SCI_FOLDLINE:=2237,SCI_FOLDCHILDREN:=2238,SCI_EXPANDCHILDREN:=2239
        ,SCI_SETMARGINCURSORN:=2248,SCI_GETMARGINCURSORN:=2249,SCI_MARKERSETBACKSELECTED:=2292
        ,SCI_MARKERENABLEHIGHLIGHT:=2293,SCI_GETEDGECOLOUR:=2364,SCI_SETWRAPINDENTMODE:=2472
        ,SCI_GETWRAPINDENTMODE:=2473,SCI_GETSELEOLFILLED:=2479,SCI_SETSELEOLFILLED:=2480
        ...
        ,SCI_CHANGELEXERSTATE:=2617,SCI_CONTRACTEDFOLDNEXT:=2618,SCI_SETIDENTIFIER:=2622
        ,SCI_GETIDENTIFIER:=2623,SCI_RGBAIMAGESETWIDTH:=2624,SCI_RGBAIMAGESETHEIGHT:=2625
        ,SCI_MARKERDEFINERGBAIMAGE:=2626,SCI_REGISTERRGBAIMAGE:=2627,SCI_SETTECHNOLOGY:=2630
        ,SCI_GETTECHNOLOGY:=2631,SCI_CREATELOADER:=2632,SCI_COUNTCHARACTERS:=2633
        ,SCI_AUTOCSETCASEINSENSITIVEBEHAVIOUR:=2634,SCI_AUTOCGETCASEINSENSITIVEBEHAVIOUR:=2635,SCI_AUTOCSETMULTI:=2636
        ,SCI_AUTOCGETMULTI:=2637,SCI_FINDINDICATORSHOW:=2640,SCI_FINDINDICATORFLASH:=2641
        ,SCI_FINDINDICATORHIDE:=2642,SCI_GETRANGEPOINTER:=2643,SCI_GETGAPPOSITION:=2644
        ,SCI_DELETERANGE:=2645,SCI_GETWORDCHARS:=2646,SCI_GETWHITESPACECHARS:=2647
        ,SCI_SETPUNCTUATIONCHARS:=2648,SCI_GETPUNCTUATIONCHARS:=2649,SCI_GETSELECTIONEMPTY:=2650
        ,SCI_RGBAIMAGESETSCALE:=2651,SCI_VCHOMEDISPLAY:=2652,SCI_VCHOMEDISPLAYEXTEND:=2653
        ,SCI_GETCARETLINEVISIBLEALWAYS:=2654,SCI_SETCARETLINEVISIBLEALWAYS:=2655,SCI_SETLINEENDTYPESALLOWED:=2656
        ,SCI_GETLINEENDTYPESALLOWED:=2657,SCI_GETLINEENDTYPESACTIVE:=2658,SCI_AUTOCSETORDER:=2660
        ,SCI_AUTOCGETORDER:=2661,SCI_FOLDALL:=2662,SCI_SETAUTOMATICFOLD:=2663
        ,SCI_GETAUTOMATICFOLD:=2664,SCI_SETREPRESENTATION:=2665,SCI_GETREPRESENTATION:=2666
        ,SCI_CLEARREPRESENTATION:=2667,SCI_SETMOUSESELECTIONRECTANGULARSWITCH:=2668,SCI_GETMOUSESELECTIONRECTANGULARSWITCH:=2669
        ,SCI_POSITIONRELATIVE:=2670,SCI_DROPSELECTIONN:=2671,SCI_CHANGEINSERTION:=2672
        ,SCI_GETPHASESDRAW:=2673,SCI_SETPHASESDRAW:=2674,SCI_CLEARTABSTOPS:=2675
        ,SCI_ADDTABSTOP:=2676,SCI_GETNEXTTABSTOP:=2677,SCI_GETIMEINTERACTION:=2678
        ,SCI_SETIMEINTERACTION:=2679,SCI_GETLEXERLANGUAGE:=4012,SCI_PRIVATELEXERCALL:=4013
        ,SCI_PROPERTYNAMES:=4014,SCI_PROPERTYTYPE:=4015,SCI_DESCRIBEPROPERTY:=4016
        ,SCI_DESCRIBEKEYWORDSETS:=4017,SCI_GETLINEENDTYPESSUPPORTED:=4018,SCI_ALLOCATESUBSTYLES:=4020
        ,SCI_GETSUBSTYLESSTART:=4021,SCI_GETSUBSTYLESLENGTH:=4022,SCI_FREESUBSTYLES:=4023
        ,SCI_SETIDENTIFIERS:=4024,SCI_DISTANCETOSECONDARYSTYLES:=4025,SCI_GETSUBSTYLEBASES:=4026
        ,SCI_GETSTYLEFROMSUBSTYLE:=4027,SCI_GETPRIMARYSTYLEFROMSTYLE:=4028
    }
}