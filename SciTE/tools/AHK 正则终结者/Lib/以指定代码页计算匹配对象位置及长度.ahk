...
以指定代码页计算匹配对象位置及长度(String, VarIn, 代码页="UTF-8")
{
  ...
  loop, % VarIn.MaxIndex()
  {
    ...
    {
      PosVarOut:="", LenVarOut:=""          ; 重设 Pos 坐标，重设 Len 长度
      ; 将匹配文本之前的所有字符捕捉出来，并转存为 UTF-8 格式，用于计算位置和长度
      ; 注意如果不直接使用 StrPut() 返回的成功写入的字符，而用 VarSetCapacity(Var, -1) 的方式，常常会出错
      VarInTemp[索引, "Pos", A_Index-1]:=StrPutVar(SubStr(String,1,VarIn[索引].Pos[A_Index-1]-1),PosVarOut,代码页)-1	
      ; 转换后的字符，注意最后一个字节总是为 0 ，所以需要减 1
      VarInTemp[索引, "Len", A_Index-1]:=StrPutVar(VarIn[索引].Value[A_Index-1],LenVarOut,代码页)-1
