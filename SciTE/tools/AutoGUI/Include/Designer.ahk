CreateToolbox(Hidden := 0) {
    ...
    Gui Add, ListView
    , hWndhLVToolbox gToolboxHandler x0 y%g_ToolbarH% w160 h%ToolboxH% %ToolboxOptions% Hidden%Hidden%
    , Control Types
...
NewGUI:
    ...
    Menu AutoEditMenu, Check, Set as &Read-Only
...
ToggleGrid:
    ...
    Repaint(hChildWnd)
    SendMessage TB_CHECKBUTTON, 1080, %g_ShowGrid%,, ahk_id %hGUIToolbar%
    Menu AutoOptionsGuiMenu, ToggleCheck, Show &Grid
Return

ToggleSnapToGrid:
    g_SnapToGrid := !g_SnapToGrid
    Menu AutoOptionsGuiMenu, ToggleCheck, S&nap To Grid
    SendMessage TB_CHECKBUTTON, 1090, %g_SnapToGrid%,, ahk_id %hGUIToolbar%
Return
...
; Credits to MJs for the resizers
CreateResizers() {
